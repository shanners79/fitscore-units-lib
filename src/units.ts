...
export interface UnitPreferences {
  mass: 'kg' | 'lb';
  distance: 'm' | 'cm' | 'in' | 'ft';        // For movement/sports distances
  length: 'cm' | 'in';                       // For body measurements (height, reach)
  time: 's' | 'min';
  speed: 'm/s' | 'km/h' | 'mph';
  // Non-convertible unit families - these are pass-through
  count: 'count';
  percent: 'percent';
  score: 'score';
  reps: 'reps';
...
export function getMetricType(metricKey: string): keyof UnitPreferences {
  ...
  if (metricKey.includes('score') || metricKey.includes('fms') || metricKey.includes('rating')) return 'score';
  if (metricKey.includes('count') || metricKey.includes('number')) return 'count';
  if (metricKey.includes('percent') || metricKey.includes('%')) return 'percent';
  if (metricKey.includes('reps') || metricKey.includes('repetitions')) return 'reps';
 ...
 */
export function isNonConvertibleUnit(unit: string): unit is 'count' | 'percent' | 'score' | 'reps' | '%' | 'level' {
 ...
 */
export function getTestDisplayUnit(
  apiUnit: string,
  baseUnitRef: { key: string; family: string } | undefined,
